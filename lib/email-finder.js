    ...
    return new Promise(function (resolve, reject) {
        ...
        var q = async.queue(function (email, callback) {
            ...
            emailExistence.check(email,function (err, res) {
                ...
                if (res) {
                    ...
                    checker_object = {status: 'success', first_name: firstname, middle_name: middlename, last_name: lastname, email: email};
                    // Kill the queue
                    q.kill();
        ...
        emailExistence.check("catchalltest123@" + domain, function (err, res) {
            ...
            if (!res) {
                ...
                checker_object = {status: 'catch-all',first_name: firstname, middle_name: middlename, last_name: lastname, email: emailsArr.slice(0,6)};
                return resolve(checker_object);
            }
        });
        ...
        q.drain = function () {
            ...
            console.log(emailsArr[1]);
            // reject();
            checker_object = {status: 'not-found',first_name: firstname, middle_name: middlename, last_name: lastname, email: emailsArr.slice(0,6)};
            return resolve(checker_object)
        }
    });
}
